...
function animation(readability, readtime, rating){
    ...
    chrome.extension.getBackgroundPage().console.log("animate");

    readabilityCircle.animate(readability);  // Number from 0.0 to 1.0
    timeCircle.animate(readtime);  // Number from 0.0 to 1.0
    ratingBar.animate(rating)
}
...
chrome.tabs.query({ 'active': true, 'windowId': chrome.windows.WINDOW_ID_CURRENT },
    function (tabs) {
        if (isValidURL(tabs[0].url)) {
            var cleanedURL = tabs[0].url.replace(/\//g, '|')
